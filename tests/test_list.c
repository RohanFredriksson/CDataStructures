...
int main() {
    ...
    if (List_Pop(&l, NULL)) {flag = 1;}
    if (List_Shift(&l, NULL)) {flag = 1;}
    ...
    if (List_Get(&l, 5, NULL) != 0) {flag = 1;}

    // Test get with invalid bounds and buffers
    if (List_Get(&l, -4, NULL) != 0) {flag = 1;}
    if (List_Get(&l, 10000, NULL) != 0) {flag = 1;}
    if (List_Get(&l, -4, &buffer) != 0) {flag = 1;}
    if (List_Get(&l, 10000, &buffer) != 0) {flag = 1;}

    // Pop all numbers off the list, should get all elements in reverse order
    for (int i = NUM_ELEMENTS-1; i >= 0; i--) {
        if (List_Pop(&l, &buffer) != 1) {flag = 1;}
    ...
    if (List_Pop(&l, NULL) != 0) {flag = 1;}
    if (List_Shift(&l, NULL) != 0) {flag = 1;}
